...
NEW_DAYS = 5
LIMIT = 200
TARGET_SLUG_PART = "pre-market"  # шукаємо тільки в crypto/pre-market
...
def parse_timestamp(created_at):
    try:
        ...
        try:
            ...
            print("❌ Timestamp parse error:", created_at, e)
            return None

def get_crypto_pre_market():
    all_markets = []
    offset = 0
    ...
    while True:
        ...
        for m in markets:
            ...
            # Перевіряємо, що це pre-market у crypto
...
def check_markets():
    global seen_markets
    markets = get_crypto_pre_market()
    if not markets:
        print(f"⚠️ Немає нових ринків crypto/pre-market за останні {NEW_DAYS} днів")
        return
    ...
    for m in markets:
        ...
        if market_id not in seen_markets:
            ...
            print("🆕 crypto/pre-market:", title)
            new_found += 1

        seen_markets.add(market_id)

    print(f"✅ Нових crypto/pre-market ринків: {new_found}")
    print("------")

# ---- Старт бота ----
print("🚀 Bot started...")
send_message(f"✅ Бот запущений — стежить за crypto/pre-market ({NEW_DAYS} днів)")
