...
def get_all_recent_markets():
    """
    Отримуємо всі ринки у pre-market
    """
    all_markets = []
    offset = 0
    ...
    while True:
        ...
        for m in markets:
            ...
            if created_dt < threshold:
                stop = True
                break
            # 🔹 Перевіряємо, що ринок дійсно має тег crypto/pre-market
            tags = [t.get("slug") for t in m.get("tags", []) if t.get("slug")]
...
def check_markets():
    ...
    if not markets:
        print(f"⚠️ Немає нових ринків за останні {NEW_DAYS} дні у pre-market")
        return
    ...
    print(f"✅ Нових ринків за останні {NEW_DAYS} дні: {new_found}")
    print("------")

# ---- Старт бота ----
print("🚀 Bot started...")
send_message(f"✅ Бот запущений і стежить за ринками у pre-market за останні {NEW_DAYS} дні")
